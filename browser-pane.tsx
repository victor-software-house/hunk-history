...
export function BrowserPane(props: ExtensionPaneProps & { sidebar: SidebarController; onMore(): void }) {
  ...
  return <box style={{ width, height, backgroundColor: theme.panel, flexDirection: "column" }}>
    <box style={{ height: 1, flexShrink: 0, flexDirection: "row" }}>
      {(["files", "history"] as const).map((view, index) => <box key={view} id={`tab-${view}`}
        style={{ width: index === 0 ? Math.floor(width / 2) : width - Math.floor(width / 2), height: 1, flexShrink: 0,
          backgroundColor: hovered === view ? theme.accentMuted : tab === view ? theme.panelAlt : theme.panel }}
        ...
        }}><text selectable={false} truncate wrapMode="none" fg={tab === view ? theme.accent : hovered === view ? theme.text : theme.muted}
          style={{ width: "100%", height: 1 }}><b>{tab === view ? " ▸ " : "   "}{view === "files" ? "Files" : "History"}</b></text></box>)}
    </box>
